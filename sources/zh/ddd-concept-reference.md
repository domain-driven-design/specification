...
对于一些信息化的应用来说，软件的设计往往是围绕着对数据的修改完成的。对于经验不多的开发者会认为，软件的开发过程就是对数据库的增删改查，这种弊病让软件难以维护。
仓库的目的是屏蔽业务逻辑和持久化基础设施之间的差异。例如，对于同样的持久化业务需求，在采用关系型数据库和非关系型数据库作为存储基础设施时的实现细节是有所不同的。
