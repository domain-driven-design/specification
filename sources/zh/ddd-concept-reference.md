...
其主要的思想是，它通过确定业务模型指导各个业务与应用的设计，强调开发人员与业务人员的不断沟通和模型的持续迭代演化，从而保证业务模型与代码模型的一致性，进而优化软件设计、解决业务复杂度。
...
软件设计是 DDD 讨论的最终目的，使用 DDD 的各种工具可以指导软件设计，最终构建出健壮、容易维护的软件来。
...
模块的划分没有固定的模式，如果不是特别复杂的业务逻辑可以使用上下文划分模块，次一级的包使用对象类型组织即可。因此，在软件工程中，对模块的说法莫衷一是，每个开发者有自己对模块的划分方法。
...
仓库是 DDD 中非常重要的概念，对于一些信息化的应用来说，软件的设计往往是围绕着对数据的修改完成的。对于经验不多的开发者会认为，软件的开发过程就是对数据库的增删改查，这种弊病让软件难以维护。
使用仓库，可以隔离业务逻辑和对象的持久化，让对象的持久化和重建工作更为纯粹，让业务逻辑有领域服务来完成。

## 服务（Service）

**服务（Service）是领域模型的操作者，承载领域的业务规则。** 

领域模型用于承载数据和系统状态，为了让逻辑更为清晰明了，业务规则的事情就交给服务。如果使用主、客的思维来看待软件设计，在系统内部，服务作为主题存在，领域模型为客体。
在这种设计中，主体使用拟人化的命名可以让开发者更容易理解。比如，一个维护客户数据的 CRM 应用中，客户数据被抽象为利于模型 Client，对应的服务可以设计为 ClientManager，让业务逻辑变的更为直观。
...
工厂是一种设计模式，在广义的工厂模式中，工厂可以更具不同的规则的业务逻辑构造不同的对象。例如在 Redis 连接客户端的实现中，可以使用 Redis 单机、哨兵、集群等不同的方式来构建 Redis 连接客户端。
...
策略模式也是一种设计模式，策略模式是面向对象原则中多态的一种体现。
...
规格常常被用来处理查询和搜索，在 JPA、MongoDB、ElastiSearch和一些具有查询能力的 ORM 都大量使用这种设计方式，同样的在应用程序中我们也可以参考这种设计模式，把业务的规则提取出来。
...
分层架构的理念和计算机网络类似，为自顶而上的架构设计。每层的职责不同，上层依赖下层。依赖，可以理解为 "信息流"，也就是被依赖方不需要知道具体依赖方的信息，提供一致的服务和能力。比如 HTTP 协议构建在 TCP 协议之上，TCP 协议只负责提供传输层的能力，而不需要知道具体的应用层协议。

分层架构中每层不是完全一定要按照四层的设计，在一些简单的系统中，可以减少分层设计，避免样板代码的冗余。在 web 系统中有时候只有一种接入方式，接入层和应用层能力高度重叠，可以考虑直接使用应用层代替接入层。

看待分层架构时，还需要注意要整体考虑。比如框架提供的能力也需要考虑在分层架构中，ORM 提供了 Repository 作为基础设施层的部分能力，需要考虑是否需要进一步封装。

## 接入层 (Interface)

**接入层，处理系统输入、输出。** 

对接入层来说，实际上接入层是依附于应用层存在的，随着前后端分离，Restful API 成了主流，对简单的系统来说这一层越来越弱化。对于有终端接入的系统来说，接入层并不简单，需要处理各种协议适配：XMPP、websocket、MQTT 等。

接入层的特点：

- 接入层对应用数据透明，只关心数据格式而不关心数据的内容
- 在大部分单体系统中接入层被框架实现了，比如在 Spring Boot 中的 Tomcat 和 Spring MVC，分布式系统中被网关实现，实现限流等能力。
